...
const fetchDepartmentsByClient = async (clientName: string): Promise<DepartmentType[]> => {
  const response = await axios.get(`${process.env.NEXT_PUBLIC_GO_APP_SERVER_URL}/departments/${clientName}`, {
  ...
  const handleSignup = async (event: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_GO_APP_SERVER_URL}/signup`,
        { ...formData, departmentID, files: filesToUpload },
        { withCredentials: true }
      )
    ...
    const uploadPromises = files.map(async file => {
      ...
      fileReader.onload = async event => {
        ...
        if (target && target.result) {
          ...
          await axios.post(`${process.env.NEXT_PUBLIC_GO_APP_SERVER_URL}/hooks/upload`, fileUploadData, {
            withCredentials: true
          })
        }
      }
  ...
  return (
    <Card>
      ...
      <form onSubmit={handleSignup}>
        ...
        <Divider />
        <CardActions>
          <Button color='primary' variant='contained' type='submit' disabled={!isValidForm()}>
            Submit
          </Button>
          <Button color='secondary' variant='tonal' onClick={handleReset}>
            Reset
          </Button>
        </CardActions>
      </form>
