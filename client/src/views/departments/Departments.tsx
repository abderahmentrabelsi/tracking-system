...
import { Grid, Dialog, DialogTitle, DialogContent, DialogActions, Button, TextField, Typography, IconButton, Box, Snackbar, Alert, InputAdornment, CircularProgress, Tooltip, Fab, MenuItem, Select } from '@mui/material';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import SearchIcon from '@mui/icons-material/Search';
import AddIcon from '@mui/icons-material/Add';
import moment from 'moment';
import ClientCard from './ClientCard';
import { fetchClients, fetchDepartments, createDepartment, updateDepartment, deleteDepartment, createClient, updateClient, deleteClient } from '@/app/api/departmentApi';
...
const Departments = () => {
    ...
    if (selectedClient) {
      const fetchDepartmentsData = async () => {
        ...
        try {
          ...
          const departmentsWithSupervisorName = await Promise.all(departmentsResponse.map(async department => {
  ...
  const handleAddDepartment = async () => {
    try {
      const newDepartment = await createDepartment({ name, clientName: selectedClient!.name, supervisorId: Number(supervisorId) });
      const supervisor = await fetchUserById(newDepartment.supervisorId);
      setDepartments(prevData => [...prevData, { ...newDepartment, supervisorName: `${supervisor.firstName} ${supervisor.lastName}` }]);
      setClients(prevClients => prevClients.map(client => client.name === selectedClient!.name ? { ...client, departmentCount: client.departmentCount + 1 } : client));
      setOpen(false);
  ...
  const handleUpdateDepartment = async () => {
    ...
    try {
      ...
      setDepartments(prevData =>
        prevData.map(dep => (dep.ID === editValue.ID ? { ...dep, name, supervisorId: Number(supervisorId), supervisorName: `${supervisor.firstName} ${supervisor.lastName}` } : dep))
      );
      setOpen(false);
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'CreatedAt', headerName: 'Created At', flex: 1, valueFormatter: (params) => moment(params.value).format('YYYY-MM-DD') },
    {
      field: 'actions',
      headerName: 'Actions',
      flex: 1,
  ...
  return (
    <>
      <Box mb={4}>
        <Typography variant='h4' gutterBottom>Organizations & Departments Management</Typography>
        ...
        <DialogContent>
          <Box mb={2} display="flex" alignItems="center">
            ...
            <Tooltip title="Add Department">
              <Fab color="primary" onClick={() => { setEditValue(null); setName(''); setSupervisorId(''); setOpen(true); }}>
                <AddIcon />
              </Fab>
            </Tooltip>
          </Box>
      ...
      <Dialog open={clientCreateOpen} onClose={() => setClientCreateOpen(false)}>
        ...
        <DialogContent>
          <TextField margin='dense' label='Client Name' fullWidth value={clientName} onChange={e => setClientName(e.target.value)} />
        </DialogContent>
        <DialogActions>
